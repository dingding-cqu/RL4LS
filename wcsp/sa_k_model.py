...
class LNSHeuristic:
    def __init__(self, pth, models, device='cpu', p=.2, scale=10):
    ...
    def step(self, select_k_assignments_by_model=True):
        k=len(self.models)
        best_cost, best_assignment = 10**6, None
        ...
        # select_k_assignments_by_model = True
        if select_k_assignments_by_model:
            self.env.set_assignments(assignment)
            for model_itr in range(k):
                self.model = self.models[model_itr]
                ...
                cost = self.total_cost(new_assignment)
                if cost < best_cost:
                    best_cost = cost
...
if __name__ == '__main__':
    d = 'cpu'
    top_k_model_id = [288,275,260,269,284,280,208,291,236,287]
    top_k_models = []
    for i in top_k_model_id:
        model_pth = '../models/'+str(i)+'.pth'
        m = GATNet(4, 16)
        m.load_state_dict(torch.load(model_pth, map_location=d))
        top_k_models.append(m)

    dirs = r'../problems/valid_model'
    bc = []
    for prob in os.listdir(dirs):
        if not prob.endswith('.xml'):
            continue
        problem = f'{dirs}/{prob}'
        lns = LNSHeuristic(problem, top_k_models, d)
        best_cost = -1
        if len(bc) != 0:
            mean = sum(bc) / len(bc)
        else:
        ...
        for _ in range(1000):
            ...
            cost = lns.total_cost(lns.assignment)
            new_cost = lns.total_cost(new_assignment)
            p_acc = math.exp(-(new_cost - cost) / T)
            if random.random() < p_acc:
                lns.assignment = dict(new_assignment)
                c = new_cost
            else:
                c = cost
