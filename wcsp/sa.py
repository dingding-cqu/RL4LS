...
class SAHeuristic:
    def __init__(self, pth, model, device='cuda:0', p=.2, scale=10):
...
if __name__ == '__main__':
    ...
    dirs = r'../problems/problems4paper/120/0.6'
    bc = []
    for prob in os.listdir(dirs):
        if not prob.endswith('.xml'):
            continue
        problem = f'{dirs}/{prob}'
        lns = SAHeuristic(problem, m, d)
        best_cost = -1
        if len(bc) != 0:
            mean = sum(bc) / len(bc)
        else:
        ...
        for current_cycle in range(1000):
            ...
            f_p.writelines(f'{current_cycle}\t {(perf_counter() - cur_t):.2f} \t{c:.2f}\t{best_cost:.2f}\n')
            # print(best_cost, c, mean, sep='\t')
        f_p.close()
        print(problem, best_cost, datetime.datetime.now())
        bc.append(best_cost)
    print("avg cost:", sum(bc)/len(bc))