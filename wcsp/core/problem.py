...
class Problem:
    ...
    def random_domain_binary(self, nb_agent, domain_list, p1, min_cost=0, max_cost=100, gc=False,
                      weighted=False, adv_domains=None, decimal=-1):
        if adv_domains is None:
            adv_domains = list()
        assert isinstance(nb_agent, int) and nb_agent > 0
        ...
        agent_domain = dict()
        for i in range(nb_agent):
            dom = random.sample(domain_list, 1)[0]
            self.add_agent(i + 1, dom)
            agent_domain[i+1] = dom
        connected = set()
        remaining = set([x for x in self.agents if self.agents[x] != 'ADV'])
        while len(remaining) > 0:
            agnt1 = random.sample(remaining, 1)[0]
            if len(connected) > 0:
                agnt2 = random.sample(connected, 1)[0]
                self.add_constraint([agnt1, agnt2], Problem._random_matrix(agent_domain[agnt1], agent_domain[agnt2],
                                                                               min_cost, max_cost, gc, weighted, decimal))
            remaining.discard(agnt1)
            connected.add(agnt1)
        edge_cnt -= (nb_agent - 1)
        ...
        while edge_cnt > 0:
            while True:
                agents = random.sample(remaining, 2)
                matrix = Problem._random_matrix(agent_domain[agents[0]], agent_domain[agents[1]], min_cost, max_cost, gc, weighted, decimal)
