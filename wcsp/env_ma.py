...
class Environment:
    def __init__(self, domains, all_functions, is_valid, device='cpu'):
    ...
    def build_graph(self, target_var, partial_assignment):
        ...
        return torch.tensor(x, dtype=torch.float32, device=self.device), \
               ...
               [node_index[target_var] + i for i in range(len(self.domains[target_var]))]
    ...
    def _dfs_build_graph(self, partial_assignment, cur_var, x, edge_index, node_index, all_function_node_index):
        ...
        src, dest = edge_index
        for val in range(len(self.domains[cur_var])):
            x.append(x_embed)
        for p in self.dfs_tree[cur_var].all_parents:
            ...
            if p in partial_assignment:
                for val in range(len(self.domains[cur_var])):
                    idx = len(x)
                    x.append(c_embed + [self.function_table[cur_var][p][self.domains[cur_var][val]][partial_assignment[p]]])
                    src.append(idx)
                    dest.append(f_idx)
                ...
                for val_p in range(len(self.domains[p])):
                    for val in range(len(self.domains[cur_var])):
                        idx = len(x)
                        x.append(c_embed + [self.function_table[cur_var][p][self.domains[cur_var][val]][self.domains[p][val_p]]])
                        src.append(idx)
                        dest.append(f_idx)
