...
x_embed = [1, 0, 0] # +[in_current_assignment, in_best_assignment ]
...
class Environment:
    ...
    def total_cost(self, assign):
        cost = 0
        for data, v1, v2 in self.all_functions:
            cost += data[assign[v1]][assign[v2]]
        return cost
        ...
        for var in self.dom_size.keys():
            ...
            for val in range(self.dom_size[var]):
                ...
                if self.best_assignment[var] == val:
                    app_feat[1] = 1
                x.append(x_embed+app_feat)
        assign_len = len(x)

        all_func_ind = []
        for data, var1, var2 in self.all_functions:
            func_ind = len(x)
            all_func_ind.append(func_ind)
            x.append(f_embed)
            var1_ind, var2_ind = var2ind[var1], var2ind[var2]
            for val1 in range(self.dom_size[var1]):
                for val2 in range(self.dom_size[var2]):
                    cost_ind = len(x)
                    x.append(c_embed+[data[val1][val2]])

                    src.append(cost_ind)
                    dest.append(func_ind)

                    src.append(var1_ind+val1)
                    dest.append(cost_ind)
                    src.append(cost_ind)
                    dest.append(var1_ind+val1)
                    src.append(var2_ind+val2)
                    dest.append(cost_ind)
                    src.append(cost_ind)
                    dest.append(var2_ind+val2)
        self.x = x
